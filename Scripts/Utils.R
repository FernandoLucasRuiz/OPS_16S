...
plotear_abundancia_relativa <- function(objeto_abundancias, titulo) {
    ...
    p <- ggplot(objeto_abundancias, aes(x = 1, y = Relative_Abundance, fill= taxa_sub)) +
        ...
        labs(title = paste0(titulo, " Relative Abundance"), 
             x = "", 
             y = ""
        ) +
        theme_minimal() +
        theme(
            ...
            legend.spacing.x = unit(0.3, 'cm'),  # Ajusta espacio entre elementos de la leyenda
            plot.title = element_text(hjust = 0.5, vjust = 2, size = 16, face = "bold"),  # Baja el título y lo pone en negrita
            plot.margin = margin(t = 10),  # Ajusta margen superior
            strip.text = element_text(face = "bold")  # Hace los títulos de facet_wrap en negrita
        ) +
...
deseq_flr <- function(objeto_tse, rara = F, prevalence, taxa_relevante, species = F) {
    ...
    for (i in variables_salida) {
        ...
        if (species == T) {
            # Combinar las columnas "Genus" y "Species"
            genus_species <- rowData(objeto_tse) %>%
                as.data.frame() |> 
                dplyr::mutate(Genus_Species = paste(Genus, Species, sep = "_"))
            
            counts <- assay(objeto_tse)
            rownames(counts) <- genus_species$Genus_Species
        ...
        if (i == "Survival") {
            metadata[[i]] <- relevel(metadata[[i]], ref = "YES")
        } else {
...
volcanos_maaslin_flr <- function(objeto_maslin){
    ...
    for (i in names(objeto_maslin)) {
        ...
        # Filtrar los 10 puntos con el coeficiente más alto
        ...
        p <- ggplot(volcano_data, aes(x=coef,
            ...
            geom_point(aes(size = Regulation, alpha = Regulation), show.legend = TRUE) +
            scale_size_manual(values = setNames(c(2, 4, 4), c("Not Significant", "Hiperbiosis" , "Hipobiosis"))) +
            scale_alpha_manual(values = setNames(c(0.1, 1, 1), c("Not Significant", "Hiperbiosis" , "Hipobiosis"))) +
            scale_color_manual(values = setNames(c("#26828EFF", "#FA8072", "#87CEFA"), c("Not Significant", "Hiperbiosis" , "Hipobiosis"))) +
            geom_text_repel(data = top_features, 
                            aes(label = feature),
                            color = "black",
                            force = 15,  # Empuja más fuerte para separar etiquetas
                            max.overlaps = Inf,  # Permite todas las etiquetas
                            direction = "both",  # Permite mover etiquetas en ambas direcciones
                            point.padding = unit(0.5, "lines"),  # Espacio alrededor de los puntos
                            box.padding = unit(0.5, "lines"),  # Espacio entre etiquetas
                            size = 4,
                            segment.size = 0.3,  # Grosor de la línea
                            segment.color = 'grey50',
                            max.segment.length = unit(0.7, "lines"),  # Hace que las flechas sean más largas
                            arrow = arrow(length = unit(0.03, "npc"), type = "closed", ends = "last")) +
            labs(title = paste0("Volcano Plot: ", i), x = "Coef. (by MaAsLin2)", y = "-Log10 P-value") + 
            theme_minimal() +
            ...
            theme(
                ...
                legend.justification = "center",                   # Centrar la leyenda horizontalmente
                legend.key.size = unit(0.5, "cm"),
                legend.text = element_text(size = 8),
                legend.title = element_blank(),
            )
...
aldex_flr <- function(objeto_tse, prevalence, taxa_relevante, species = F) {
    ...
    for (i in variables_salida) {
        ...
        if (species == T) {
            ...
            rownames(selex.sub) <- genus_species$Genus_Species
...
ancombc2_flr <- function(objeto_tse, prevalence, taxa_relevante, species = F) {
    ...
    if (species == T) {
        ...
        rownames(objeto_tse) <- genus_species$Genus_Species
